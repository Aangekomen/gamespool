...
<div class="max-w-lg mx-auto">
    <h1 class="text-2xl font-bold mb-1"><?= e($title) ?></h1>
    <p class="text-slate-400 text-sm mb-6">Geef het spel een naam en kies hoe je punten telt.</p>
    ...
    <form method="post" action="<?= e($action) ?>" class="space-y-4">
        ...
        <div>
            <label class="block text-sm font-medium mb-1.5" for="name">Naam</label>
        ...
        <div>
            <label class="block text-sm font-medium mb-1.5" for="score_type">Scoresysteem</label>
        ...
        <!-- Per-type config -->
        <div data-config="win_loss" class="<?= $currentType === 'win_loss' ? '' : 'hidden' ?> space-y-3 rounded-lg border border-slate-800 bg-slate-900/40 p-4">
            <p class="text-sm text-slate-400">Vaste punten per uitslag (klassiek voetbal-style).</p>
            <div class="grid grid-cols-3 gap-3">
                <?php foreach (['win_points' => 'Winst', 'draw_points' => 'Gelijk', 'loss_points' => 'Verlies'] as $k => $label):
                    ...
                    <label class="block">
                        <span class="block text-xs text-slate-400 mb-1"><?= e($label) ?></span>
                        <input type="number" name="score_config[<?= e($k) ?>]" value="<?= e((string) $val) ?>" min="0" max="100"
                               class="w-full rounded-md bg-slate-900 border border-slate-700 px-2 py-2 text-base">
                    </label>
                <?php endforeach; ?>
            </div>
        </div>

        <div data-config="points_per_match" class="<?= $currentType === 'points_per_match' ? '' : 'hidden' ?> rounded-lg border border-slate-800 bg-slate-900/40 p-4">
            <p class="text-sm text-slate-400">De ingevoerde score per match telt rechtstreeks als punten (bijv. 7 ballen gepot = 7 punten).</p>
        </div>

        <div data-config="elo" class="<?= $currentType === 'elo' ? '' : 'hidden' ?> space-y-3 rounded-lg border border-slate-800 bg-slate-900/40 p-4">
            <p class="text-sm text-slate-400">Elo-rating per speler. Sterkere tegenstander winnen levert meer op.</p>
            <div class="grid grid-cols-2 gap-3">
                <?php $elo = $currentType === 'elo' ? $config : Game::defaultConfig('elo'); ?>
                <label class="block">
                    <span class="block text-xs text-slate-400 mb-1">Startrating</span>
                    <input type="number" name="score_config[start_rating]" value="<?= e((string) ($elo['start_rating'] ?? 1000)) ?>" min="100" max="3000"
                           class="w-full rounded-md bg-slate-900 border border-slate-700 px-2 py-2 text-base">
                </label>
                <label class="block">
                    <span class="block text-xs text-slate-400 mb-1">K-factor</span>
        ...
        <div class="flex items-center gap-3 pt-2">
            ...
            <a href="<?= e(url('/games')) ?>" class="px-4 py-3 rounded-lg bg-slate-800 hover:bg-slate-700">Annuleren</a>
        </div>
    </form>
</div>
