...
<div class="mb-4 flex items-center justify-between">
    <div>
        <h1 class="text-2xl font-bold text-navy">Gebruikers</h1>
        <p class="text-slate-500 text-sm"><?= count($users) ?> account<?= count($users) === 1 ? '' : 's' ?> geregistreerd.</p>
    </div>
    <a href="<?= e(url('/admin')) ?>" class="text-sm text-slate-500 hover:text-navy">← admin</a>
</div>

<?php if (empty($users)): ?>
    <p class="text-slate-500 text-center py-8">Nog geen gebruikers.</p>
<?php else: ?>
    <ul class="space-y-2">
        ...
        ?>
            <li class="flex items-center gap-3 rounded-xl border border-slate-200 bg-white px-4 py-3 shadow-card">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <p class="text-xs text-slate-500 truncate"><?= e((string) $u['email']) ?>
                ...
                <div class="text-right shrink-0">
                    <p class="text-sm font-bold text-navy tabular-nums"><?= (int) $u['matches_played'] ?></p>
