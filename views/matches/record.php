...
<div class="max-w-lg mx-auto">
    <h1 class="text-2xl font-bold text-navy mb-1">Uitslag invoeren</h1>
    ...
    <form method="post" action="<?= e(url('/matches/' . $match['id'] . '/record')) ?>"
          class="space-y-3 bg-white p-5 rounded-xl border border-slate-200 shadow-card">
        <?= csrf_field() ?>
            ...
            ?>
                <div class="rounded-xl border border-slate-200 bg-surface p-3 flex items-center gap-3">
                    ...
                    <input type="number" name="p[<?= e((string) $p['id']) ?>][raw_score]"
                           ...
                           class="w-24 rounded-lg bg-white border border-slate-300 px-3 py-2.5 text-base text-right focus:outline-none focus:ring-2 focus:ring-brand"
                           placeholder="0">
                </div>
            <?php endforeach; ?>
        ...
        <?php else: /* win_loss + elo: pick exactly one winner OR mark all as draw */ ?>
            ...
            <button type="button" id="drawBtn"
                    class="w-full mt-1 px-4 py-2 rounded-md text-sm font-medium bg-slate-100 hover:bg-slate-200 text-slate-700">
                Of: gelijkspel
            </button>
        <?php endif; ?>
    ...
    <form method="post" action="<?= e(url('/matches/' . $match['id'] . '/cancel')) ?>"
        ...
        <?= csrf_field() ?>
        <button type="submit" class="w-full px-4 py-3 rounded-lg bg-white border border-slate-200 hover:bg-red-50 hover:text-red-700 text-slate-600">
            Match annuleren
        </button>
    </form>
</div>
...
<script>
    ...
    function clearPickStyles() {
        ...
        $draw.classList.add('bg-slate-100', 'hover:bg-slate-200', 'text-slate-700');
    }
    ...
    $draw.addEventListener('click', () => {
        clearPickStyles();
        $draw.classList.remove('bg-slate-100', 'hover:bg-slate-200', 'text-slate-700');
